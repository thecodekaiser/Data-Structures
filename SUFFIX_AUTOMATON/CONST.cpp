...
void solve()
{
	...
	{
		cout << w << " is not a substring of " << s << " but " << cnt << " chars match.\n";
		...
		for(int i = cnt; i < ss.size(); i++)
		{
			if(a.edges[n].find(ss[i]) == a.edges[n].end())
			{
				fail = true; break;
			}
			n = a.edges[n][ss[i]];
		...
		if(!fail)
			cout << ss << " is a substring of " << s << endl;
		else
			cout << ss << " is not a substring of " << s << " but " << ncnt << " chars match.\n";
